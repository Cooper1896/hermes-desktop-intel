...
Please go to [Releases](https://www.google.com/search?q=../../releases) to download the corresponding version of the artifacts:
...
> **Note**: Apple Silicon (M1/M2/M3, etc.) users should directly use the official release versions.
...
Python (>= 3.11, required for running the Agent core)
* Git
Git
...
Build artifacts will be outputted to the `dist/` directory. For details, please refer to [docs/BUILD.md](https://www.google.com/search?q=docs/BUILD.md).
...
# After a successful compilation, please re-commit the Submodule pointer changes
...
.github/workflows  # GitHub Actions 自动化流水线（在 Push 或 Tag 时触发 x64 构建）
                   # GitHub Actions automation workflow (triggers x64 builds on Push or Tag)
...
* **签名与公证**：Releases 中提供的安装包均采用 **Ad-hoc 签名且未经 Apple 官方公证（Notarization）**。请勿将其视作官方签名的正式分发包。
**Signing & Notarization**: Artifacts provided in Releases use **Ad-hoc signing and have not been officially notarized by Apple**. Do not treat them as officially signed distribution packages.
* **安全校验**：为确保制品完整性，建议在下载后手动比对 `SHA256SUMS` 哈希值。
**Security Verification**: To ensure artifact integrity, it is recommended to manually verify the `SHA256SUMS` hashes after downloading.
...
The upstream project [hermes-agent](https://github.com/NousResearch/hermes-agent) is also under the MIT License, with copyrights belonging to the original authors.
